$(document).ready(function () {
	...
	table = $('#datatable').DataTable({
		...
		ajax: {
			method: 'POST',
			url: "?pagina=ver_pagoscursos&metodo=listar"
		},
		columns: [
			{ data: 'fecha' },
			{ data: 'pago_total' },
...
function cambiarEstado(accion, id) {
	$.ajax({
		type: 'POST',
		url: `?pagina=ver_pagoscursos&metodo=${accion}&id=${id}`,
...
$('body').on('click', '.inactivar', function (e) {//Al presionar inactivar
	...
	Swal.fire({
		...
		text: "El Pago de la Cita será inactivado en el sistema",
		type: 'warning',
...
$('body').on('click', '.activar', function (e) {//Al presionar activar
	...
	Swal.fire({
		title: '¿Desea continuar?',
		text: "El Pago de la Cita será activado en el sistema",
		type: 'warning',
		showCancelButton: true,
		cancelButtonText: 'Cancelar',
		confirmButtonText: 'Si'
